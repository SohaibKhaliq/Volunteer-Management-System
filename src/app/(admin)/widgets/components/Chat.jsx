...
const Chat = () => {
  return <Card>
      ...
      height: 380
    }} id="chat-container" data-simplebar>
                {chatData.map((chat, index) => <div key={index} className={`d-flex align-items-start gap-2 my-3 chat-item ${chat.type === "outgoing" ? "text-end justify-content-end" : ""}`}>
                        {chat.type === "incoming" && <Image src={chat.avatar} className="avatar-md rounded-circle" alt={chat.user} />}

                        <div>
                            <div className={`chat-message py-2 px-3 rounded ${chat.type === "outgoing" ? "bg-info-subtle" : "bg-warning-subtle"}`}>
                                {chat.message}
                            </div>
                        ...
                        {chat.type === "outgoing" && <Image src={chat.avatar} className="avatar-md rounded-circle" alt={chat.user} />}
                    </div>)}
            </SimplebarClient>
