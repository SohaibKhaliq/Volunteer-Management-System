...
const Customizer = () => {
  ...
  return <Offcanvas show={isCustomizerOpen} onHide={toggleCustomizer} placement="end" className="overflow-hidden">
          ...
          <Row className="g-3">
            {skinOptions.map((item, idx) => <Col sm={6} key={idx}>
                <div className="form-check card-radio">
                  <input id={`skin-${item.skin}`} className="form-check-input" type="radio" name="data-skin" disabled={item.disabled ?? false} value={item.skin} checked={skin === item.skin} onChange={() => updateSettings({
                skin: item.skin
              })} />
        ...
        <div className="p-3 border-bottom border-dashed">
          ...
          <Row className="g-3">
            {themeOptions.map((item, idx) => <Col sm={4} key={idx}>
                <div className="form-check card-radio">
                  <input id={`theme-${item.theme}`} className="form-check-input" type="radio" name="data-bs-theme" value={item.theme} checked={theme === item.theme} onChange={() => updateSettings({
                theme: item.theme
              })} />
        ...
        <div className="p-3 border-bottom border-dashed">
          ...
          <Row className="g-3">
            {topBarColorOptions.map((item, idx) => <Col sm={4} key={idx}>
                <div className="form-check card-radio">
                  <input id={`topbar-color-${item.color}`} className="form-check-input" type="radio" name="data-topbar-color" value={item.color} checked={topBarColor === item.color} onChange={() => updateSettings({
        ...
        <div className="p-3 border-bottom border-dashed">
          ...
          <Row className="g-3">
            {orientationOptions.map((item, idx) => <Col sm={4} key={idx}>
                <div className="form-check card-radio">
                  <input id={`layout-${item.orientation}`} className="form-check-input" type="radio" name="data-layout" value={item.orientation} checked={orientation === item.orientation} onChange={() => updateSettings({
        ...
        <div className="p-3 border-bottom border-dashed">
          ...
          <Row className="g-3">
            {sidenavColorOptions.map((item, idx) => <Col sm={4} key={idx}>
                <div className="form-check card-radio">
                  <input id={`sidenav-color-${item.color}`} className="form-check-input" type="radio" name="data-menu-color" value={item.color} checked={sidenavColor === item.color} onChange={() => updateSettings({
                sidenavColor: item.color
              })} />
            ...
            <div className="p-3 border-bottom border-dashed">
              ...
              <Row className="g-3">
                {sidenavSizeOptions.map((item, idx) => <Col sm={4} key={idx}>
                    <div className="form-check card-radio">
                      <input id={`sidenav-size-${item.size}`} className="form-check-input" type="radio" name="data-sidenav-size" value={item.size} checked={sidenavSize === item.size} onChange={() => updateSettings({
                  sidenavSize: item.size
                })} />
              ...
              <div className="d-flex justify-content-between align-items-center">
                ...
                <div className="btn-group radio" role="group">
                  {layoutPositionOptions.map((item, idx) => <Fragment key={idx}>
                      <input type="radio" className="btn-check" name="data-layout-position" id={`position-${item.position}`} value={item.position} checked={position === item.position} onChange={() => updateSettings({
            ...
            <div className="p-3">
              <div className="d-flex justify-content-between align-items-center">
                ...
                <div className="form-check form-switch fs-lg">
                  <input type="checkbox" className="form-check-input" name="sidebar-user" checked={sidenavUser} onChange={() => updateSettings({
